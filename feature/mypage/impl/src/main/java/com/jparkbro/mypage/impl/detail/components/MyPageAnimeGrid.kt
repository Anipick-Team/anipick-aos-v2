...
private const val MYPAGE_ANIME_GRID_COLUMNS = 3
private const val MYPAGE_ANIME_GRID_SKELETON_ITEM_COUNT = 9
...
/** 마이페이지 디테일 - 볼 애니/보는 중/다 본 애니/좋아요한 작품 공통 3열 고정 그리드. */
@Composable
internal fun MyPageAnimeGrid(
    animes: List<Anime>,
    totalCount: Int,
    isLoading: Boolean,
    ...
    /** 카드 캡션 자리를 대체 - null이면 [AniPickAnimeCard] 기본(subtitle)을 그대로 쓴다. */
    cardDescription: (@Composable (Anime) -> Unit)? = null,
) {
    BoxWithConstraints(modifier = modifier) {
        val layout = remember(maxWidth) {
            calculateAnimeGridLayout(
                availableWidth = maxWidth - MYPAGE_GRID_HORIZONTAL_PADDING * 2,
                minColumns = MYPAGE_ANIME_GRID_COLUMNS,
                maxColumns = MYPAGE_ANIME_GRID_COLUMNS,
            )
        }
        ...
        LazyVerticalGrid(
            ...
            if (isLoading) {
                items(MYPAGE_ANIME_GRID_SKELETON_ITEM_COUNT) {
                ...
                items(animes, key = { it.animeId ?: it.hashCode() }) { anime ->
                    AniPickAnimeCard(
                        ...
                        descriptionContent = cardDescription?.let { { it(anime) } },
                    )
                }
